...
bible_original = open(os.path.join("data","total_data.txt"),"r").read().split("\n")
...
VISUALIZE = False
def generate_sample(font,num_samples,save_dir):
...
def gen_all_samples_from_fonts(num_samples,save_dir):
    for item in tqdm(os.listdir("fonts")):
        if(item.endswith(".ttf") and "Anothershabby" not in item):
            ...
            gen_samples(font,num_samples,save_dir)
